...
app.get("/movies", function (req, res) {
  request.get(`http://${backend_url}/movies`).end(function (err, data) {
    if (data.status == 403) {
...
app.get("/publications", function (req, res) {
  request.get(`http://${backend_url}/publications`).end(function (err, data) {
    if (data.status == 403) {
      res.send(403, "403 Forbidden");
    } else {
...
app.get("/pending", function (req, res) {
  request.get(`http://${backend_url}/pending`).end(function (err, data) {
    if (data.status == 403) {
      res.send(403, "403 Forbidden");
    }
  });
